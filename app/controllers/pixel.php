...
if (!in_array($_SERVER['REMOTE_ADDR'], $not_allowed_ips) && isset($_GET['from'], $_GET['to'], $_GET['time'], $_GET['key'])) {
    ...
    $cache_key = PIXEL_CACHE_PREFIX . $calculated_signature . '_' . Utils::slugifyIp($_SERVER['REMOTE_ADDR']);
    $cached = $redis->get($cache_key);
    if ($cached === false && $calculated_signature === $_GET['key'] && preg_match("#\@justauth\.me$#", $_GET['from'])) {
        $message = $_GET['to'] . ' opened an e-mail ' . date('Y-m-d \a\t H:i:s', Utils::time()) . ' from ' . $_SERVER['REMOTE_ADDR'];
        $mailer = new Mailer();
        $mailer->queueMail(
            $_GET['from'],
            $message,
